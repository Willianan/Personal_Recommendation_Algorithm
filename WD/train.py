...
def input_fn(data_file,re_time,shuffle,batch_num,predict):
	...
	_CSV_COLUMNS = ['age','workclass','fnlwgt','education','education-num','marital-status','occupation','relationship',
	                'race','sex','capital-gain','capital-loss','hours-per-week','native-country','label']
	def parse_csv(value):
...
def build_model_estimator(wide_column, deep_column, model_folder):
	...
	"""
	model_es = tf.contrib.learn.DNNLinearCombinedClassifier(
...
def train_wd_model(model_es, train_file, test_file, model_export_folder, serving_input_fn):
	...
	:param train_file:
	:param test_file:
	:param model_export_folder: model export for tf serving
...
def get_test_label(test_file):
	if not os.path.exists(test_file):
		return []
	fp = open(test_file,encoding="utf-8")
	linenum = 0
	test_label_list = []
	for line in fp:
		if linenum == 0:
...
def run_train(train_file,test_file,model_folder,model_export_folder):
	"""
	:param train_file:
	:param test_file:
	:param model_folder: origin model floder to put train model
...
if __name__ == "__main__":
	run_train(".../data/adult.data","../data/adult.test","../data/wd","../data/wd_export")